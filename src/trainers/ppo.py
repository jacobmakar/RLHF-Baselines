...
class PPOTrainer:
    ...
    def train(self, batch_size, num_batches):
        ...
        for batch in train_loader:
            ...
            prompts = [
                self.tokenizer.decode(prompt, skip_special_tokens=True)
                for prompt in prompts_encoded["input_ids"]
            ]
            prompts_encoded = {
                k: v.to(self.devices[0]) for k, v in prompts_encoded.items()
            }
            model_to_use = (
                self.model.module
            ...
            prompts = [
                prompt for prompt in prompts for _ in range(self.num_return_sequences)
            ]
            with torch.no_grad():
                completions = [
            ...
            output_sequences = self.tokenizer(
            ...
            initial_log_probs = compute_log_probs(
                model_logits,
            ...
            for i in range(self.num_ppo_steps):
                ...
                critic_values = critic_values.view(
                    batch_size * self.num_return_sequences
                )

                ratios = (model_log_probs - initial_log_probs).exp()

                advantages = (
                    torch.tensor(rewards).to(self.devices[0])
                    - self.beta * (model_log_probs - ref_log_probs)
                    - critic_values
                )
                clipped = (
