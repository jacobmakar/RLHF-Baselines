    ...
    def train(self, batch_size, num_batches):
        ...
        for batch in train_loader:
            ...
            output_sequences = output_sequences.view(
                batch_size, self.num_return_sequences, self.sequence_len
            )
            with torch.no_grad():
                ...
                rewards_by_prompt = [
                    list(map(lambda c: self.reward_func(prompt, c), completions))
                    for prompt, completions in zip(prompts, completions_by_prompt)
                ]
            ...
            good_sequences_encoded = self.tokenizer(
                preference_batch["winning"], return_tensors="pt", padding=True
            ).to(self.devices[0])
            bad_sequences_encoded = self.tokenizer(
                preference_batch["losing"], return_tensors="pt", padding=True
            ).to(self.devices[0])
