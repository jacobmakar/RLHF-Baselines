...
def concat_sequences(seq1, seq2, seq_len):
    truncate_len = min(
        seq1["input_ids"].size()[1], seq2["input_ids"].size()[1], seq_len
    )
    input_ids = torch.cat(
        (seq1["input_ids"][:, :truncate_len], seq2["input_ids"][:, :truncate_len]),
        dim=0,
    )
    attention_mask = torch.cat(
        (
            seq1["attention_mask"][:, :truncate_len],
            seq2["attention_mask"][:, :truncate_len],
        ),
        dim=0,
    )
...
def preference_loss(
    model, input_ids, attention_mask, return_outputs=False, rewards=None
):
    if rewards is None:
        rewards = model(input_ids=input_ids, attention_mask=attention_mask)[0]
    batch_size = input_ids.size(0) // 2
    rewards_chosen, rewards_rejected = torch.split(
        rewards, [batch_size, batch_size], dim=0
    )
...
def policy_loss(
    ...
    if model_log_probs is None or model_logits is None:
        ...
        model_log_probs = compute_log_probs(
            model_logits, labels, prompt_len, pad_token_id
        )
...
def train_main_model(trainer, batch_size, num_batches):
    ...
    for batch in train_loader:
        ...
        prompts_encoded = trainer.tokenizer(
            batch["text"],
            return_tensors="pt",
            max_length=trainer.prompt_len,
            padding=True,
            truncation=True,
        ).to(trainer.devices[0])
        prompts = [
            trainer.tokenizer.decode(prompt, skip_special_tokens=True)
            for prompt in prompts_encoded["input_ids"]
        ]
        prompts_encoded = {
            k: v.to(trainer.devices[0]) for k, v in prompts_encoded.items()
        }
        model_to_use = (
        ...
        completions = [
            trainer.tokenizer.decode(output_sequence, skip_special_tokens=True)
        ...
        output_sequences = trainer.tokenizer(
            completions, return_tensors="pt", padding=True
        ).to(trainer.devices[0])

        with torch.no_grad():
            preference_scores = trainer.preference_model(
                input_ids=output_sequences["input_ids"],
                attention_mask=output_sequences["attention_mask"],
            )[0]
        ...
        loss, kl = policy_loss(
            ...
            output_sequences["attention_mask"],
            output_sequences["input_ids"],
            preference_scores,
            trainer.ref_model,
            trainer.beta,
        ...
        wandb.log(
            {
                ...
                "policy_eq_check/examples": len(train_loader) * batch_size
                + count * batch_size,
            }
        )
...
def train_preference_model(trainer, batch_size, num_batches):
    ...
    for batch in train_loader:
        ...
        output_sequences = output_sequences.view(
            batch_size, trainer.num_return_sequences, trainer.sequence_len
        )
        with torch.no_grad():
            ...
            completions_by_prompt = [
                [c[len(prompt) :] for c in completions]
                for prompt, completions in zip(prompts, completions_by_prompt)
            ]
            rewards_by_prompt = [
                list(map(lambda c: trainer.reward_func(prompt, c), completions))
                for prompt, completions in zip(prompts, completions_by_prompt)
            ]
        ...
        good_sequences_encoded = trainer.tokenizer(
            good_completions, return_tensors="pt", padding=True
        ).to(trainer.devices[0])
        bad_sequences_encoded = trainer.tokenizer(
            bad_completions, return_tensors="pt", padding=True
        ).to(trainer.devices[0])
        input_ids, attention_mask = concat_sequences(
            good_sequences_encoded, bad_sequences_encoded, trainer.sequence_len
        )

        # Preference learning
        pref_loss, rewards = preference_loss(
            trainer.preference_model, input_ids, attention_mask, return_outputs=True
        )
        rewards_chosen, rewards_rejected = (
            rewards["rewards_chosen"],
            rewards["rewards_rejected"],
        )
        ...
        print(
            f"Inner Loop: Iteration {count}, Pref Loss: {pref_loss.item():.4f}, Chosen Reward: {avg_chosen_reward}, Rejected Reward: {avg_rejected_reward}, Accuracy: {accuracy}"
        )
