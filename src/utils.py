...
class RewardModel(nn.Module):
    def __init__(self, base_model, device, model_type):
        ...
        if isinstance(base_model, OPTForCausalLM):
            ...
            raise ValueError(
                "Unsupported model type. Please use OPTForCausalLM or GPT2LMHeadModel."
            )
        self.score_head = nn.Linear(base_model.config.hidden_size, 1, bias=False).to(
            device
        )
    ...
    def forward(self, input_ids, attention_mask):
        ...
        last_token_hidden = last_hidden_state[
            torch.arange(last_hidden_state.shape[0]), attention_mask.sum(dim=1) - 1
        ]
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for eval_batch in test_loader:
            ...
            kl_div = F.kl_div(
                pi_log_probs, ref_probs, reduction="batchmean", log_target=False
            )
            ...
            prompts_encoded = {
                k: v.to(model_device) for k, v in prompts_encoded.items()
            }  # devices[0]
            prompts = [
